...
func TestAgentRepoLifecycle(t *testing.T) {
	...
	a, err := agent.New(r, "prov-1", agent.TypeClusterProvider,
		map[string]string{"team": "infra", "franz.default-kafka-config/partitions": "3"}, "hash-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || got.FRN.String() != "frn:default:agent:prov-1" || got.Type != agent.TypeClusterProvider ||
		got.TokenHash != "hash-1" || got.Labels["franz.default-kafka-config/partitions"] != "3" {
		t.Fatalf("round-trip: %+v (%v)", got, err)
	}

	// labels are replaced wholesale via Mutate
	if _, err := repo.Mutate(ctx, r.ID, "prov-1", func(a *agent.Agent) error {
		a.Labels = map[string]string{"franz.default-kafka-config/kafka-version": "3.9.0"}
		return nil
	}); err != nil {
		t.Fatalf("mutate labels: %v", err)
	}
	if got, _ = repo.Get(ctx, r.ID, "prov-1"); got.Labels["franz.default-kafka-config/kafka-version"] != "3.9.0" ||
		got.Labels["team"] != "" {
