...
Location: `franz/cmd/localkafkaagent/` + `franz/pkg/localkafkaagent/`
...
| 07.3 | Recipe engine (`pkg/localkafkaagent/recipe`) — `local-docker`: intent + `connection_strings` + selected `cluster_configuration` → a container spec (`apache/kafka:<version>`, KRaft combined, `advertised.listeners` from the bootstrap URL); compute `franz.recipe-hash` | ADR §5 | ✅ | 2026-09-06 |
| 07.4 | Docker driver (`pkg/localkafkaagent/docker`) — Engine API SDK: create/start/stop/remove, `ContainerInspect` for state, `ContainerList` by `franz.managed-by=<agent>` label; manage the data volume | ADR §6 | ✅ | 2026-09-06 |
| 07.5 | Reconcile (`pkg/localkafkaagent/reconcile`) — per cluster: no container → create; hash mismatch → recreate; `PAUSED` → stop; `REMOVED` → stop + remove + volume; orphan container (no assignment) → remove. Idempotent; a full re-sync disturbs nothing correct | ADR §6, lifecycle table | ✅ | 2026-09-06 |
...
2. **07.9 tests — fake Docker for logic + local-only real smoke.**
   ...
   Docker driver (in CI). `pkg/localkafkaagent/e2e_test.go` is the real-Docker
...
3. **Local dev — `make agent`** uses a fixed dev bearer token installed by the
   **DB seed** (`franz/local/seed/01-local-agent.sql`, applied by `make deps`),
   ...
   _(An earlier `FRANZ_REGISTER=1` self-register path — `pkg/localkafkaagent/register.go` —
...
| Piece | Path |
|---|---|
| Binary | `cmd/localkafkaagent/main.go` |
| Config + orchestrator + Bearer creds | `pkg/localkafkaagent/{config.go,agent.go}` |
| Assignment value type | `pkg/localkafkaagent/assign` |
| Stream loop (reconnect + backoff + debounce + resync) | `pkg/localkafkaagent/stream` |
| `local-docker` recipe + hash + allow-list | `pkg/localkafkaagent/recipe` |
| Docker driver — interface + Engine API + in-memory fake | `pkg/localkafkaagent/docker` |
| Reconcile (create / recreate-on-hash / pause / remove / orphan) | `pkg/localkafkaagent/reconcile` |
| Readiness probe (franz-go Ping) | `pkg/localkafkaagent/probe` |
| Real-Docker e2e | `pkg/localkafkaagent/e2e_test.go` |
...
`pkg/localkafkaagent/register.go` (the agent created/rotated its own registration on
