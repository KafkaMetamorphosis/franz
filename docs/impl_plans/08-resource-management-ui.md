...
| 08.4 | **`useUpdateAgent` / `useUpdateKafkaCluster` hooks** — build `update_mask` from changed fields only (camelCase JSON paths, comma-joined), invalidate detail + list, surface `ApiError` (400 immutable, 404, 409) | proto, 03.4, 04.3 | ✅ | 2026-09-06 |
