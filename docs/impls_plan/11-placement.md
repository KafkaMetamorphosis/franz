...
| 11.3 | **Materialise + place shards** (ADR-API-009): on channel create/update (`franz.*` label change) and on cluster label/`state` change, run selection and — for each shard index `0..channel_partitions-1` that has no live `kafka_topic` row — **create** the row (`topic.New`, `state = PENDING`) on the selected cluster, seeding `partitions` / `replication_factor` / `materialized_configuration` from that cluster's `cluster_configuration` (003.6 OQ1: which keys). A shard already placed on a still-matching cluster is left alone | `003.7`, `003.6` | ⬜ | |
| 11.4 | Retry sweep (~30s, configurable) — for every `ACTIVE` channel with fewer live shards than `channel_partitions`, re-run selection and create the missing shards when a cluster becomes eligible | `003.7` | ⬜ | |
| 11.5 | Misplaced detection — a placed shard whose cluster no longer satisfies the channel's affinity (or went `PAUSED`/`DELETED`) gets a `misplaced` marker on the row; **no move** (that is 16) | `003.7` | ⬜ | |
| 11.6 | Absent `franz.affinity/selector` ⇒ no candidates ⇒ **no shards created** (do not reject the channel create) | `003.7` | ⬜ | |
| 11.7 | Unit + integration tests — determinism (same labels + clusters ⇒ same assignment), taint filtering, a channel with no eligible cluster has 0 shards → registering a matching cluster materialises `channel_partitions` shards, `shard-size` capping | — | ⬜ | |
