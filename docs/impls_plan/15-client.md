...
Status: ⬜ not started
Depends on: [02](./02-domain-foundations.md) · [13](./13-telemetry-ingest.md) (for observed groups)
...
**[16](./16-access-policy-and-channel-access.md)** — it needs the access-policy
...
| 15.5 | `ListObservedConsumerGroups` (current view: distinct `(group, topic)` latest sighting) + `ListConsumerGroupObservations` (raw sightings, time range) — reads the deliverable 13 tables | `003.10`, `003.14` | ⬜ | |
| 15.6 | Integration tests — realm-wide uniqueness, delete keeps the FRN reserved, observed-group views | — | ⬜ | |
